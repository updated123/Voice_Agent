...
- **Backchannel detection ("mm-hmm," "okay") is a text-keyword match against already-transcribed audio, not real overlapping-audio detection** (`libs/voice_agent_core/orchestrator/call_session.py`'s `is_backchannel`). A live system needs to decide whether to keep talking *before* it has a transcript of what was said over it — this prototype's scripted, sequential-audio model doesn't have real overlapping speech to detect, so the keyword match is an honest stand-in for the decision, not the production mechanism.
- **The mid-sentence disfluency pause fix (semantic endpointing) is architecture-only, not implemented in `libs/voice_agent_core`** — `HangoverVAD` is still pure acoustic silence-duration detection; the completeness-check-against-partial-transcript design in `services/vad-service/main.py` was never ported into the real, tested VAD backend. Backchannel detection got the equivalent real-code treatment because it was a self-contained logic fix; this one needs an actual model (even a small one) and was left at the same documented-but-not-implemented level as the rest of `services/`.
