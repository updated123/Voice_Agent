...
| **Encryption at rest** | Session records (`session-manager`/Aerospike), call transcripts and recordings (`analytics`), and account data (loan servicing backend) encrypted at rest; access scoped by least-privilege IAM roles per service — see [../infrastructure/terraform/](../infrastructure/terraform/). |
