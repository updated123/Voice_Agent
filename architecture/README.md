...
| `single-call-flow.html` | **Rendered, ready to open — the most detailed artifact in this repo.** Two parts: (1) the complete call lifecycle, campaign selection through the live consent/DND gate, dialing, AMD, the opening disclosure, every exit a call can take, and the async post-call analytics/billing/QA loop; (2) the turn-level deep dive — every cache check (session state, intent, compliance FSM, TTS phrase), every timeout/retry, and the escalation branch that permanently exits the flow. Also includes full detail cards and scaling data for all 12 services, and a sync-vs-async table for every inter-service call. No external dependencies — hand-drawn inline SVG, opens in any browser, matches both light/dark themes. |
