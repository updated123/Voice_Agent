...
- **Every service is stateless except `session-manager` — with one deliberate exception.** Call state (FSM state, slots, transcript) lives in one externalized store, keyed by call-id, so any coordinator/logic service can crash and be replaced mid-call without losing the call. `vad-service` and `denoiser` are the exception: they hold genuinely stream-local audio state (hangover-window timing, noise-floor estimate) that only makes sense in-memory for the life of one call, sticky-routed to that call's media server rather than externalized — externalizing per-frame audio state to Aerospike would add latency for no benefit, since it's cheaper to just re-route to a fresh instance and let it restart cold than to persist and reload state that rebuilds in milliseconds anyway.
